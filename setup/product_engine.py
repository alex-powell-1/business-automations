...
class Product:
        ...
        if response is not None:
            for x in response:
                ...
                if self.quantity_available - self.buffer < 0:
                    ...
                    self.buffered_quantity_available = (
                        self.quantity_available - self.buffer
                    )
    ...
    def set_buffer(self, buffer):
        ...
        if buffer == initial_buffer:
            print(
                f"Buffer for item: {self.item_no} - {self.long_descr} already at {self.buffer}"
            )
            return
        else:
            query = f"""
        ...
        for x in ecomm_items:
            ...
            if sku in binding_ids:
                ...
                if total_sold_all_children > 0:
                    bc_update_product(
                        product_id, {"total_sold": total_sold_all_children}
                    )
                    print(
        ...
        for y in items:
            for letter in range(len(y[1]) - 1):
                ...
                if (
                    ...
                    df = pandas.DataFrame(
                        log_data, columns=["item_no", "html_description"]
                    )
                    df.to_csv(
                        creds.description_log, mode="a", header=False, index=False
                    )
                    break
