...
def bc_create_product(name, product_type, sku, weight, price):
    ...
    payload = {'name': name, 'type': product_type, 'sku': sku, 'weight': weight, 'price': price}
...
def bc_get_custom_fields(product_id):
    url = (
        f' https://api.bigcommerce.com/stores/{creds.big_store_hash}/v3/catalog/products/{product_id}/custom-fields'
    )
...
def get_modifier(product_id, pretty=False):
    if product_id is not None:
        url = (
            f' https://api.bigcommerce.com/stores/{creds.big_store_hash}/v3/catalog/products/{product_id}/modifiers'
        )
        response = requests.get(url, headers=creds.bc_api_headers)
        if response.status_code == 200:
            if pretty:
        ...
        if parent is not None:
            ...
            if product_id is not None:
                ...
                if not bc_has_product_thumbnail(product_id):
                    print(
                        f'Missing Thumbnail Found for Binding Key: {key} / Product ID: {product_id}!', file=log_file
                    )
