...
class Catalog:
                ...
                if filename not in ['Thumbs.db', 'desktop.ini', '.DS_Store']:
                    ...
                    if '^' in filename:
                        if filename.split('.')[0].split('^')[1].isdigit():
                            all_files.append(
                                [filename, os.path.getsize(f'{creds.Company.product_images}/{filename}')]
                            )
                    else:
                        all_files.append([filename, os.path.getsize(f'{creds.Company.product_images}/{filename}')])
    ...
    class CategoryTree:
        ...
        def get_cp_updates(self):
            print('Getting CP Updates')
            query = """
            SELECT cp.CATEG_ID, ISNULL(cp.PARENT_ID, 0), cp.DESCR, cp.DISP_SEQ_NO, 
            ...
            if response:
                for x in response:
                    ...
                    cp_categ_id = x[0]
                    print(f'CP Categ ID: {cp_categ_id}')
                    if cp_categ_id == '0':
                    ...
                    if cp_categ_id is None:
                        ...
                        continue
                    lst_maint_dt = x[4]
                    print(f'Last Maintenance Date: {lst_maint_dt}')
                    sn_cp_categ_id = x[5]
                    is_visible = x[6]

                    if sn_cp_categ_id is None:
                        print(f'Inserting: {cp_categ_id} for {x[2]}')
                        # Insert new records
                        cp_parent_id = x[1]
                        category_name = x[2]
                        sort_order = x[3]
                        ...
                        SORT_ORDER, IS_VISIBLE, LST_MAINT_DT)
        ...
        def create_tree_in_memory(self):
            def get_categories():
                query = """
                SELECT CATEG_ID, ISNULL(PARENT_ID, 0), DESCR, DISP_SEQ_NO, LST_MAINT_DT
                FROM EC_CATEG
                WHERE CATEG_ID != '0'
                """
                response = Database.query(query)
        ...
        def sync(self):
            def build_tree(category):
                ...
                if category.lst_maint_dt > self.last_sync:
                    ...
                    if category.bc_categ_id is None:
                        print(f'Getting BC ID for {category.category_name}')
                        category.get_bc_id()

                    if category.bc_parent_id is None:
                        print(f'Getting BC Parent ID for {category.category_name}')
                        category.bc_parent_id = category.get_bc_parent_id()

                    print(f'Parent ID: {category.bc_parent_id}')
            ...
            for x in self.heads:
                print(f'Building Tree for {x.category_name}, CP ID: {x.cp_categ_id}')
                build_tree(x)
        ...
        def delete_category(self, cp_categ_id):
            ...
            if response:
                ...
                if bc_category_id is not None:
                    ...
                    response = BCRequests.delete(url=url)
        ...
        class Category:
            ...
            def get_bc_id(self):
                ...
                if response is not None:
                    ...
                    if bc_category_id is not None:
                        print(
                            f'Category {self.category_name} already exists in BigCommerce. BC ID: {bc_category_id}'
                        )
                        self.bc_categ_id = response[0][0]
                    else:
                        print(f'Category {self.category_name} not found in BigCommerce. OUT 1')
                        self.bc_categ_id = self.bc_create_category()
                        print(f'Category {self.category_name} created in BigCommerce. BC ID: {self.bc_categ_id}')
                else:
                    print(f'Category {self.category_name} not found in Middleware. OUT 2')
            ...
            def get_category_payload(self):
                ...
                if self.bc_categ_id is not None:
                    payload[0]['category_id'] = self.bc_categ_id
            ...
            def bc_update_category(self):
                ...
                response = BCRequests.put(url=url, json=payload)
            ...
            def write_category_to_middleware(self):
                query = f"""
                INSERT INTO SN_CATEG (BC_CATEG_ID, CP_CATEG_ID, CP_PARENT_ID, CATEG_NAME, SORT_ORDER, IS_VISIBLE)
    ...
    class Brands:
        ...
        def update_brand_timestamps(last_run):
            ...
            def get_brand_photos_local():
                result_list = []
                # Iterate over all files in the directory
                for filename in os.listdir(creds.Company.brand_images):
                    if filename not in ['Thumbs.db', 'desktop.ini', '.DS_Store']:
                        file_size = os.path.getsize(f'{creds.Company.brand_images}/{filename}')
        ...
        class Brand:
            def __init__(self, cp_brand_id, description, last_maint_dt, last_sync):
                ...
                self.image_url = None
                self.image_filepath = f'{creds.Company.brand_images}/{self.cp_brand_id}.jpg'
            ...
            def create(self):
                def create_bc_brand():
                    ...
                    response = BCRequests.post(url=url, json=self.construct_payload())
            ...
            def update(self):
                def update_bc_brand():
                    ...
                    response = BCRequests.put(url=url, json=self.construct_payload())
                    if response.status_code in [200, 207]:
    ...
    class Product:
            ...
            if check_for_invalid_brand:
                ...
                if self.brand:
                    ...
                    if self.validation_retries > 0:
                        self.reset_brand()
                        self.validation_retries -= 1
                        self.brand = creds.Company.product_brand
                    Catalog.logger.warn(message)
        ...
        def process(self, retries=3):
            ...
            def create(retry=1):
                ...
                if retry >= 0:
                    create_response = self.post_product()
                    if create_response.status_code in [200, 201, 207]:
                        self.get_product_data_from_response(create_response)
        ...
        def get_product_data_from_response(self, response):
            ...
            custom_field_response = response.json()['data']['custom_fields']
            ...
            variant_data = response.json()['data']['variants']
            # Get Variant ID from SKU
            if variant_data:
                for variant in response.json()['data']['variants']:
                    for child in self.variants:
                        if child.sku == variant['sku']:
                            child.variant_id = variant['id']

            # Option Value IDs
            option_data = response.json()['data']['options']
            if option_data:
            ...
            # Update Image IDs
            image_response = response.json()['data']['images']
        ...
        def bc_post_image(self, image):
            ...
            bc_response = BCRequests.post(url=url, json=image_payload)
        ...
        def bc_update_product(self, payload):
            ...
            bc_response = BCRequests.put(url=url, json=payload)
        ...
        def bc_get_custom_fields(self):
            ...
            cf_response = BCRequests.get(url=cf_url)
        ...
        def bc_delete_product_option_value(self, product_id, option_id, value_id):
            ...
            response = BCRequests.delete(url)
            return response
        ...
        def bc_create_product_variant_option_value(self, variant_name, product_id, option_id):
            ...
            response = BCRequests.post(url=url, json=value_payload)
            if response.status_code == 200:
                return response.json()['data']['id']
        ...
        def delete_product(self, sku, binding_id=None):
            ...
            del_product_response = BCRequests.delete(url=delete_url)
        ...
        def delete_variant(self, sku, binding_id=None):
            ...
            if self.is_last_variant(binding_id=binding_id):
                ...
                while rate_limited:
                    ...
                    del_variant_response = BCRequests.delete(url=url)
        ...
        def insert_variant(self, variant):
            ...
            insert_query = (
                f"INSERT INTO {table} (ITEM_NO, BINDING_ID, IS_PARENT, "
            ...
            if insert_product_response['code'] == 200:
                ...
                Catalog.error_handler.add_error_v(
                    error=f'Query: {insert_query}\n\nResponse: {insert_product_response}',
        ...
        class Variant:
            ...
            def bc_post_variant(self, product_id=None):
                ...
                bc_response = BCRequests.post(url=url, json=variant_payload)
            ...
            def insert_variant(self, variant):
                ...
                if insert_product_response['code'] == 200:
                    ...
                    Catalog.error_handler.add_error_v(
                        error=f'QUERY: {insert_query}\n\nRESPONSE: {insert_product_response}',
                        origin=f'SKU: {variant.sku} Bind: {variant.binding_id} - Variant --> insert_variant',
                    )
                return insert_product_response
            ...
            def get_bc_product_images(self):
                ...
                response = BCRequests.get(url=url)
                if response is not None:
        ...
        class Image:
            def __init__(self, image_name: str):
                ...
                self.sku = ''
                self.file_path = f'{creds.Company.product_images}/{self.image_name}'
                self.image_url = ''
                self.product_id = None
                self.variant_id = None
                self.image_id = None
