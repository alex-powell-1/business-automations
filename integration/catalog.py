...
class Catalog:
        ...
        if add_targets:
            ...
            for x in add_targets:
                ...
                if product_id is not None:
                    variant = Catalog.Product.Variant(sku=variant_sku, last_run_date=self.last_sync, mode=self.mode)
    ...
    def sync(self, initial=False):
        ...
        if not self.sync_queue:
            ...
            while len(self.sync_queue) > 0:
                ...
                prod = self.Product(target, last_sync=self.last_sync, mode=self.mode)
    ...
    def delete_product(self, sku, update_timestamp=False):
        ...
        product = Catalog.Product(product_data=delete_payload, last_sync=self.last_sync, mode=self.mode)
    ...
    class Product:
        def __init__(self, product_data, last_sync, mode='BigCommerce'):
            self.db = Database.db
            self.mode = mode
                ...
                def get_binding_id_images():
                    ...
                    if total_binding_images > 0:
                        ...
                        for image in binding_images:
                            binding_img = self.Image(image, mode=self.mode)
            ...
            def construct_image_payload():
                ...
                for image in self.images:
                    if self.mode == 'BigCommerce':
                        ...
                        image_payload = {'position': image.sort_order, 'src': image.image_url, 'variant_ids': [{}]}
            ...
            def construct_variant_payload():
                ...
                if self.mode == 'BigCommerce':
                    ...
                    if self.is_bound:
                        for child in self.variants:
                            variant_payload = {
                                'title': child.variant_name,
                                'compare_at_price': child.price_1,
                                'price': min(child.price_1, child.price_2),
                                'cost': child.cost,
                                'sku': child.sku,
                                'position': self.variants.index(child) + 1,
                                'option1': child.variant_name,
                                'inventory_policy': 'deny',
                                'fulfillment_service': 'manual',
                                'inventory_management': 'shopify',
                                'updated_at': convert_to_utc(datetime.now()),
                                'taxable': False,
                                'grams': 0,
                                'weight': child.weight,
                                'weight_unit': 'lb',
                                'inventory_quantity': child.buffered_quantity,
                                'requires_shipping': False,
                            }

                            if child.product_id:
                                variant_payload['product_id'] = child.product_id
                            if child.variant_id:
                                variant_payload['id'] = child.variant_id
                        ...
                        variant_payload = {
                            'title': 'Default Title',
                            'compare_at_price': self.default_price,
                            ...
                            'weight': self.weight,
                            'weight_unit': 'lb',
                            'inventory_quantity': self.buffered_quantity,
                            'requires_shipping': False,
                        }
                        if self.product_id:
                            variant_payload['product_id'] = self.product_id
            ...
            if self.mode == 'BigCommerce':
                ...
                payload = {
                    'product': {
                        ...
                        'body_html': self.html_description,
                        'vendor': self.brand,
                        'product_type': self.custom_type,
                        'handle': Catalog.parse_custom_url_string(self.web_title),
                        'published_scope': 'global',
                        'tags': self.search_keywords,
                ...
                if self.is_bound:
                    payload['product']['options'] = [
                        {'name': 'Option', 'position': 1, 'values': [x.variant_name for x in self.variants]}
                    ]
                else:
                    payload['product']['options'] = [{'name': 'Title', 'position': 1, 'values': 'Default Title'}]
        ...
        def post_product(self):
            ...
            if self.mode == 'BigCommerce':
                ...
                response = Shopify.create_product(payload=self.construct_product_payload())
                print(f'Shopify Response: {response}')
                if response[0]:
                    return response[1]
        ...
        class Variant:
            def __init__(self, sku, last_run_date, get_images=True, mode='BigCommerce'):
                self.db = Database.db
                self.mode = mode
                self.sku = sku
            ...
            def get_local_product_images(self):
                ...
                if total_images > 0:
                    ...
                    for image in product_images:
                        img = Catalog.Product.Image(image_name=image, mode=self.mode)
                        if img.validate():
        ...
        class Image:
            def __init__(self, image_name: str, mode='BigCommerce'):
                self.db = Database.db
                self.mode = mode
                self.id = None
...
if __name__ == '__main__':
    ...
    cat = Catalog(last_sync=datetime(year=2024, month=7, day=15, hour=8, minute=20), mode='Shopify')
    cat.sync()
