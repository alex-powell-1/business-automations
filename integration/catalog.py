    ...
    def process_deletes():
        ...
        all_cp_products = [
            ...
            for x in db.query_db(
                f"SELECT ITEM_NO FROM IM_ITEM WHERE IS_ECOMM_ITEM = 'Y'"
            )
            if x
        ]
        all_mw_products = [
    ...
    class Product:
                ...
                if response is not None:
                    ...
                    for item in response:
                        self.variants.append(
                            self.Variant(item[0], last_run_date=self.last_sync)
                        )
                ...
                def get_binding_id_images():
                    ...
                    if total_binding_images > 0:
                        ...
                        for image in binding_images:
                            binding_img = self.Image(
                                image, last_run_time=self.last_sync
                            )
        ...
        def validate_product_inputs(self):
            ...
            if check_for_missing_brand:
                ...
                if self.brand == "":
                    message = (
                        f"Product {self.sku} is missing a brand. Validation failed."
                    )
                    self.errors.append(message)
                    print(message)
                    return False
            ...
            def create():
                def bc_create_product():
                    ...
                    if bc_response.status_code in [200, 207]:
                        ...
                        for x in range(0, len(self.variants)):
                            ...
                            self.variants[x].variant_id = bc_response.json()["data"][
                                "variants"
                            ][x]["id"]
                        ...
                        for x in range(0, len(self.images)):
                            ...
                            self.images[x].image_id = bc_response.json()["data"][
                                "images"
                            ][x]["id"]
                ...
                def middleware_insert_images():
                    ...
                    for image in self.images:
                        query = (
                            f"INSERT INTO {creds.bc_image_table} (IMAGE_NAME, ITEM_NO, FILE_PATH, IMAGE_URL, "
                        ...
                        try:
                            insert_image_response = query_engine.QueryEngine().query_db(
                                query, commit=True
                            )
                        except Exception as e:
                ...
                def bc_update_product(payload):
                    ...
                    if update_response.status_code in [200, 201, 207]:
                        ...
                        for x in range(0, len(self.variants)):
                            self.variants[x].binding_id = self.binding_id
                            self.variants[x].product_id = self.product_id
                            self.variants[x].variant_id = update_response.json()[
                                "data"
                            ]["variants"][x]["id"]
                        for x in range(0, len(self.images)):
                            self.images[x].binding_id = self.binding_id
                            self.images[x].product_id = self.product_id
                            self.images[x].image_id = update_response.json()["data"][
                                "images"
                            ][x]["id"]
                        if self.custom_field_response:
                            custom_field_list = []
                ...
                def middleware_sync_images():
                    ...
                    for image in self.images:
                        if image.id is None:
                            ...
                            try:
                                insert_img_response = (
                                    query_engine.QueryEngine().query_db(
        ...
        def delete_image(self, target):
            ...
            image_id = self.get_image_id(target)
            delete_img_url = (
                f"https://api.bigcommerce.com/stores/{creds.test_big_store_hash}/v3/"
            ...
            if img_del_res.status_code == 204:
                print(f"{target} deleted from BigCommerce")
                sql_del_image = f"""
                DELETE FROM {creds.bc_image_table}
                WHERE IMAGE_NAME = '{target}'"""
                try:
                    self.db.query_db(sql_del_image, commit=True)
                except Exception as e:
                    message = (
                        f"Image: {target} Failed to Delete from SQL ",
                        e,
                    )
                    print(message)
                    self.errors.append(message)
                else:
                    print(f"{target} deleted from SQL")
        ...
        class Variant:
            ...
            def get_variant_details(self):
                ...
                Enabled(1)', ISNULL(ITEM.IS_ADM_TKT, 'N') as 'Is Parent(2)', ISNULL(BC_PROD.PRODUCT_ID, 
                '') as 'Product ID (3)', ISNULL(BC_PROD.VARIANT_ID, '') as 'Variant ID(4)', ITEM.USR_CPC_IS_ENABLED 
                ...
                if item is not None:
                    details = {
                        ...
                        "cost": item[0][47],
                        "categories": str(item[0][49]).split(",")
                        if item[0][49]
                        else [],
                        "custom_url": "",
                        "is_custom_url": False,
