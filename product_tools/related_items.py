...
def set_related_items_by_category():
	error_handler.logger.info(f'Setting related items: Starting at {datetime.now():%H:%M:%S}')
	...
	for x in categories:
		...
		popular_items = product_reports.create_top_items_report(
			beginning_date=date_presets.two_weeks_ago,
		...
		if related_items is not None:
			...
			if products is not None:
				for z in products:
					big_products.bc_update_product(get_bc_product_id(z), payload=payload)
					error_handler.logger.info(f'Cat: {x}, {count}/{len(products)}, Updated Item: {z}')
					count += 1

	error_handler.logger.info(f'Setting related items: Completed at {datetime.now():%H:%M:%S}')
